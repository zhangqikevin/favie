...
- **Skill versions do not propagate on their own.** Despite the SDK note that unpinned agents follow new versions, agents created with `skills[]` on `createAgent` kept the version current at creation (v9 / v25 / v31 observed on 09-16 with v35 live). `publishOperatingPrompt` now re-attaches the skill to every agent with `versionPin: null` and verifies via `listAgentSkills`; `syncSkillToAgents()` can be run alone.
- **Cron runs lose their final reply (seen 2026-09-17/18).** ZooWork finishes cron-triggered runs with `outputType: not_required` and no final `agent.assistant` event, so the favie-summary never arrives; an API-triggered follow-up in the same session answers normally. `collectRun` therefore nudges a report-less daily run once. The nudged row stays `finished` (never `running`: it blocked the 08:00 disputes checks for a day) and is re-collected by our own row, not by cron discovery — the nudge flips the session's channel from `cron` to `api`.
- Scheduled dispute checks: a busy agent writes a retryable `agent_busy` row; a missed 08:00 tick is picked up by later hours when the store was already connected before 08:00 local.
