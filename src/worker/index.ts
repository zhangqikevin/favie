...
await boss.work<{ restaurantId: string; platform: 'uber_eats' | 'doordash'; mode: 'check' | 'process' | 'fast'; orderId?: string | null }>(JOBS.disputesManual, { batchSize: 3, pollingIntervalSeconds: 0.5 }, async (jobs) => {
  await Promise.all(jobs.map(async (job) => {
