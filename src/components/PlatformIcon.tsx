...
export function PlatformIcon({ platform, className = 'h-12 w-12 rounded-2xl', muted = false }: { platform: Platform; className?: string; muted?: boolean }) {
  if (platform === 'uber_eats') {
    return (
      <svg viewBox="0 0 48 48" className={`overflow-hidden ${className}`} aria-label="Uber Eats" role="img">
        <rect width="48" height="48" fill={muted ? '#cfd5e0' : '#0b0f19'} />
  ...
  return (
    <svg viewBox="0 0 48 48" className={`overflow-hidden ${className}`} aria-label="DoorDash" role="img">
      <rect width="48" height="48" fill={muted ? '#cfd5e0' : '#ff3008'} />
