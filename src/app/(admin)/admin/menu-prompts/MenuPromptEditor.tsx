...
import { saveMenuPrompt, resetMenuPrompt, type AdminState } from '../actions'

/** One editable prompt: read-only until the edit switch is on; Save stores it, Reset returns to the built-in default. */
export function MenuPromptEditor({ which, title, help, value, defaultValue, updatedAt, preview, footnote }: {
  which: 'describe' | 'image'
  title: string
  help: string
  value: string | null
  defaultValue: string
  ...
  const [state, action, pending] = useActionState<AdminState, FormData>(saveMenuPrompt, undefined)
  ...
  return (
    <form action={action} className={`card p-6 ${editing ? 'ring-2 ring-amber-300' : ''}`}>
      ...
      <textarea name="prompt" value={draft} onChange={(e) => setDraft(e.target.value)} readOnly={!editing} rows={8} spellCheck={false}
        className={`input mt-4 min-h-[180px] font-mono text-[13px] leading-relaxed ${editing ? '' : 'cursor-default bg-ink-100/60 text-ink-700'}`} />
      {preview && !editing && (
        <details className="mt-3">
          <summary className="cursor-pointer text-xs text-ink-500">Preview with a sample dish</summary>
      ...
      {editing && (
        <div className="mt-3 flex flex-wrap items-center gap-3">
          ...
          {value && (
            <button formAction={resetMenuPrompt} formNoValidate disabled={pending} onClick={(e) => { if (!window.confirm('Go back to the built-in default prompt?')) e.preventDefault() }} className="text-xs text-ink-500 hover:text-ink-900 hover:underline">Reset to default</button>
          )}
