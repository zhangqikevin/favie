...
import { runNow, runDisputesNow, type AdminState } from '../actions'
...
export function RunDisputesNow({ restaurantId }: { restaurantId: string }) {
  const [state, action, pending] = useActionState<AdminState, FormData>(runDisputesNow, undefined)
  return (
    <form action={action} className="card flex flex-wrap items-center gap-3 p-5">
      <input type="hidden" name="restaurantId" value={restaurantId} />
