...
/** Monthly or yearly (11 months' price). The chosen plan travels to the checkout action as a hidden field. */
  ...
  return (
    <form action={action}>
        ...
        <div role="radiogroup" aria-label={t('ob.billing.plan')} className="mb-5 grid grid-cols-2 gap-2">
          {(['monthly', 'yearly'] as const).map((p) => (
            <button key={p} type="button" role="radio" aria-checked={plan === p} onClick={() => setPlan(p)}
              ...
              <span className="mt-0.5 block text-xs text-ink-500">{p === 'monthly' ? t('ob.billing.monthly.sub', { price: usd(MONTHLY_USD) }) : t('ob.billing.yearly.sub', { price: usd(YEARLY_PER_MONTH_USD) })}</span>
      ...
      <div className="mt-1 flex items-baseline gap-1">
        <span className="font-display text-5xl font-bold tracking-tight tabular-nums">{usd(yearly ? YEARLY_USD : MONTHLY_USD)}</span>
        <span className="text-ink-500">{yearly ? t('common.year') : t('common.month')}</span>
      </div>
