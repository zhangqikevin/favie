...
async function assertMenuUnlocked(restaurantId: string) {
  if (await openOptimization(restaurantId)) throw new Error('menu locked: Favie is optimizing this menu')
}
export async function requestMenuOptimization(restaurantId: string) {
  const { r, user } = await own(restaurantId)
...
export async function updateDraft(menuItemId: string, patch: { description?: string | null; imageUrl?: string | null }) {
  ...
  await assertMenuUnlocked(item.restaurantId)
