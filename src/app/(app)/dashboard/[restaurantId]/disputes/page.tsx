            ...
            {manualRuns.map((run) => {
              ...
              const head = (
                <div className="flex flex-wrap items-center gap-x-4 gap-y-1 py-3">
                  ...
                  {run.channel?.split(':')[3] === 'fast' && <span className="rounded-full bg-brand-50 px-2 py-0.5 text-xs font-semibold text-brand-700">{t('disp.manual.fast')}</span>}
                  {run.finishedAt && run.startedAt && <span className="text-xs tabular-nums text-ink-400">{Math.max(1, Math.round((run.finishedAt.getTime() - run.startedAt.getTime()) / 1000))}s</span>}
