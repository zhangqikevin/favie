...
export default async function LandingPage() {
  ...
  return (
    <main>
      ...
      {/* HOW IT WORKS: vertical timeline + the one step that matters (the secure login) as a visual */}
        ...
        <div className="container-x grid items-center gap-14 lg:grid-cols-12">
          <div className="lg:col-span-5">
            ...
            <ol className="mt-10 relative">
              <span aria-hidden="true" className="absolute left-[15px] top-3 bottom-3 w-px bg-ink-300/60" />
              {([1, 2, 3] as const).map((i) => (
                <li key={i} className="relative flex gap-5 pb-9 last:pb-0">
                  <span className={`font-display relative z-10 flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-sm font-bold ${i === 3 ? 'bg-brand-500 text-white' : 'bg-white text-ink-900 ring-1 ring-ink-300/60'}`}>{i}</span>
                  <div className="pt-0.5">
                    <div className="flex flex-wrap items-center gap-2">
                      <h3 className="font-display text-lg font-bold">{t(`how.${i}.t`)}</h3>
                      <span className="rounded-full bg-white px-2 py-0.5 text-[11px] font-medium text-ink-500 ring-1 ring-ink-100">{t(`how.${i}.time`)}</span>
                    </div>
                    <p className="mt-1.5 max-w-md leading-relaxed text-ink-500">{t(`how.${i}.b`)}</p>
                  </div>
                </li>
              ))}
            </ol>
          </div>
          {/* Two portal login windows, handed to the owner; a lock badge carries the promise. */}
            ...
            <div className="relative">
              <div className="card relative ml-auto w-[88%] p-0 shadow-xl">
                <div className="flex items-center gap-2 border-b border-ink-100 px-4 py-2.5">
                  ...
                  <span className="ml-3 truncate rounded-md bg-ink-100 px-2 py-0.5 text-[11px] text-ink-500">merchant-portal.doordash.com</span>
                </div>
                <div className="flex items-center gap-4 px-6 py-7">
                  <PlatformIcon platform="doordash" className="h-12 w-12 rounded-xl" />
                  <div className="flex-1 space-y-2.5">
                    <div className="h-2.5 w-2/5 rounded bg-ink-100" /><div className="h-9 rounded-lg bg-ink-100/70" /><div className="h-9 rounded-lg bg-ink-100/70" />
                  </div>
                </div>
              </div>
              <div className="card relative -mt-16 w-[88%] p-0 shadow-2xl">
                <div className="flex items-center gap-2 border-b border-ink-100 px-4 py-2.5">
                  <span className="h-2.5 w-2.5 rounded-full bg-ink-300/70" /><span className="h-2.5 w-2.5 rounded-full bg-ink-300/70" /><span className="h-2.5 w-2.5 rounded-full bg-ink-300/70" />
                  <span className="ml-3 truncate rounded-md bg-ink-100 px-2 py-0.5 text-[11px] text-ink-500">merchants.ubereats.com/manager</span>
                </div>
                <div className="flex items-center gap-4 px-6 py-7">
                  <PlatformIcon platform="uber_eats" className="h-12 w-12 rounded-xl" />
                  <div className="flex-1 space-y-2.5">
                    <div className="h-2.5 w-2/5 rounded bg-ink-100" /><div className="h-9 rounded-lg bg-ink-100/70" /><div className="h-9 w-1/3 rounded-lg bg-ink-900" />
                  </div>
                </div>
              </div>
